...
void bfs(list target, map<vector<vector<int>>,vector<vector<int>>>& list_s){
    if(target.x-1>-1){
...
void bin_bfs(list target,list& src,int &sum, map<vector<vector<int>>,vector<vector<int>>>& list_s){
    bfs(src, list_s);
...
int main(){
    ...
    list src;
    test_src(src);
    map<vector<vector<int>>,vector<vector<int>>> list_s;
    return 0;
}
